...
- Restrict who can write to the queue, budget/quota the provider key, set spending alerts, and review any existing backlog. Concurrency three is not rate limiting or authorization.
...
The event detail page loads the linked job and renders completed output through the existing private-R2 image endpoint. Pending and failed states reuse the shared generated-image status component. Saving an outcome invalidates the route loader so the new job appears immediately; later queue progress still requires a browser refresh because this spike adds no polling.

Event resolution is persisted first and remains authoritative. Queue send failure is stored as `enqueue_failed`, but there is an unavoidable best-effort gap if D1 fails between resolution and creation of the linked job. That rare state is shown as a missing image job rather than failing the already-committed immutable resolution and encouraging an unsafe retry. There is no repair/regenerate control in this spike. The resolution RPC and event-image status RPC share the existing unauthenticated-spike limitation and must be protected before production paid generation.
