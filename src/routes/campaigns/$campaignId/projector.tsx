...
export const Route = createFileRoute("/campaigns/$campaignId/projector")({
	ssr: false,
...
function ProjectorPage() {
	...
	const initialInput = useMemo(
		() => readCollections(collections, campaignId),
	...
	useEffect(() => {
		...
		const poll = async () => {
			...
			try {
				...
				const nextInput = readCollections(collections, campaignId);
	...
	}, [campaignId, collections, loadProjectorImages]);
...
function readCollections(
	...
	const warbands = collections.warbands.toArray.filter(
		(warband) => warband.campaignId === campaignId,
	);
	const warriors = collections.warriors.toArray.filter(
		(warrior) => warrior.campaignId === campaignId,
	);
	const matches = collections.matches.toArray.filter(
		(match) => match.campaignId === campaignId,
	);
	const matchIds = new Set(matches.map((match) => match.id));
	return {
		events: collections.events.toArray.filter(
			(event) => event.campaignId === campaignId,
		),
		matches,
		participants: collections.warbandMatches.toArray.filter((participant) =>
