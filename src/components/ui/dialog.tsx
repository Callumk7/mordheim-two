...
export const dialogContentVariants = cva(
	"fixed top-1/2 left-1/2 z-50 grid w-full max-h-[calc(100vh-2rem)] max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 overflow-y-auto border gap-6 rounded-4xl bg-popover p-6 text-sm text-popover-foreground ring-1 ring-foreground/5 duration-100 outline-none data-entering:animate-in data-entering:fade-in-0 data-entering:zoom-in-95 data-exiting:animate-out data-exiting:fade-out-0 data-exiting:zoom-out-95",
	{
		variants: {
			size: {
	...
	return (
		<DialogOverlay isDismissable={isDismissable} {...props}>
			...
			>
				<DialogPrimitive
					data-slot="dialog"
					className="[display:inherit] [gap:inherit] min-w-0 outline-none"
				>
					{children}
					{showCloseButton && (
						<DialogClose
